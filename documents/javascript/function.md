...
```
    ...
    이름을 가진 함수는 그 함수가 선언된 함수 내부 전체에서 유효하다.
...
### 1.3.1 인자에서 함수의 매개변수까지
    함수를 호출할떄 인자 목록을 제공하면 이 인자들은 함수를 정의할 때 지정한 매개변수에
    순서대로 할당된다. 인자 개수와 매개변수의 개수가 다르더라도 에러는 방생하지 않는다.
    ...
    this 매개변수는 함수 호출과 관계된 객체를 참조히며, 이를 함수 콘텍스트라고 한다.
...
### 1.3.4 생성자로 호출
    ...
    첫째, 비어있는 객체가 생성된다,
    둘째, 새로 생성된 객체는 this 매게변수로 생성자 함수에 전달되고, 생성자 함수의 함수 콘텍스트가 된다.
